# 2026-07-20-004：刷新 browser benchmark 趋势样本
...
browser benchmark history 需要持续补样，避免旧样本继续主导趋势判断。当前这轮刷新是在组件 pending guard 之后拉新窗口，再据此选择下一轮最窄的性能切片。
...
| `solace-project-log/solace-entries/2026-07-20-004-browser-benchmark-trend-refresh.md` | 新增 | 记录本次趋势刷新            |
...
| Full browser history      | `pnpm benchmark:history -- --min-browser-count 20 --json`                                                                                  | 通过，browser records 30；median: 8.15 / 3.5 / 1.2 ms                              |
| Latest browser history    | `pnpm benchmark:history -- --latest-browser-count 5 --min-browser-count 5 --json`                                                          | 通过，latest browser records 5；median: 7.1 / 3.2 / 1.2 ms                         |
