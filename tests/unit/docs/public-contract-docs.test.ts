...
describe("public contract documentation", () => {
  it("keeps release gates and deferred beta boundaries aligned", async () => {
    ...
    expect(projectStatus).toContain("Repository package version: `0.1.0-beta.8`");
    expect(projectStatus).toContain("Published npm `beta`: `0.1.0-beta.8`");
    ...
    expect(projectStatusZh).toContain("仓库 package 版本：`0.1.0-beta.8`");
    expect(projectStatusZh).toContain("npm `beta` 已发布版本：`0.1.0-beta.8`");
    ...
    expect(projectStatus).toContain("86.33% branches");
    ...
    expect(projectStatusZh).toContain("86.33% branches");
    expect(projectStatusZh).toMatch(/93\.23%\s+functions/);
    expect(projectStatusZh).toMatch(/90\.94%\s+lines/);
    expect(projectStatus).toMatch(/routine Node 20\/22\s+CI now runs `pnpm adoption:smoke`/);
    expect(projectStatus).toContain("`.github/workflows/performance-history.yml`");
    expect(projectStatus).toContain("pnpm package:devtools-extension");
    expect(projectStatus).toContain("no real production origin has been verified");
    expect(projectStatusZh).toMatch(/常规 Node 20\/22 CI 现在会执行\s+`pnpm adoption:smoke`/);
    expect(projectStatusZh).toContain("`.github/workflows/performance-history.yml`");
    expect(projectStatusZh).toContain("pnpm package:devtools-extension");
    expect(projectStatusZh).toContain("尚未验证真实生产 origin");
    ...
    expect(migration).toContain("pnpm adoption:evidence");
    expect(migration).toMatch(/does\s+not execute their recorded `argv`/);
    expect(migration).toMatch(/does\s+not\s+count as 1\.0 evidence/);
    expect(migrationZh).toContain("pnpm adoption:evidence");
    expect(migrationZh).toContain("不会执行其中的 `argv`");
    expect(migrationZh).toContain("不能作为 1.0 证据");
