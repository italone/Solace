...
location 的 query value 必须是 string、number、boolean、null、undefined，或这些值的数组。
导航到当前 `fullPath`，或导航到最终 redirect 回当前 `fullPath` 的 route 时，会解析为当前 route，且不会写入
...
会补前导 `/`、移除除 `/` 外的 path 尾随斜杠，并保留 query string。`push()` 和 `replace()`
更新浏览器状态，但不会直接调用 listener。
