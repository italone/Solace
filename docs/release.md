...
and release script drift fail early. The routine quality matrix runs `pnpm adoption:smoke` against the
packed candidate on both supported Node lines. CI also runs `pnpm release:contract:check` and emits the
...
The scheduled `.github/workflows/performance-history.yml` workflow runs once per UTC day. It restores
the latest successful `.benchmark-history/` cache, appends jsdom and Chromium records, generates a
structured evidence snapshot, and uploads the accumulated history for 30 days. Cache keys are unique
per run and restore by prefix, so each successful run carries prior dates forward without mutating
source control. After at least five independent dates are present, review the uploaded JSONL, copy the
verified history into the release-evidence process, and regenerate `release/performance-history.json`.
The schedule itself is collection infrastructure, not proof that five dates have already elapsed.
...
An independent application may declare a repository-relative `adoptionEvidenceBundle` generated by
`pnpm adoption:evidence`. The loader rebuilds its baseline/candidate/rollback bundle, verifies the
bundle SHA-256, and the evaluator binds exact versions, application identity, repository, production
origin, production workflows, reviewer approval, and rollback restoration to the application record.
Current fixtures and compatibility applications declare no such production bundle and remain excluded
from the independent adoption count.
