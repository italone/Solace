...
| SSR/hydration   | buffered async、顺序流式加 router 组合  | `renderToStringAsync()` 会把完整 async initial tree 解析为 buffered HTML/styles；`hydrateAsync()` 会在触碰 server DOM 前完成准备，并支持显式 DOM mismatch recovery。现有同步 API 保留同步返回类型并拒绝未解析 async values。`renderToStream()` 已实现顺序流式（sequential）streaming SSR（beta）：字节顺序与 `renderToStringAsync().html` 一致，async component 解析前先刷新已完成前缀，样式在首次注册处内联发射；渲染 eager 启动且不处理消费者 backpressure。`createRouterServerContext()` 增加 request-scoped memory-router settlement、canonical snapshot serialization 和 verify-before-hydrate 组合。直接 `router` option 拒绝保持不变；乱序 streaming 和 renderer-owned router integration 仍 deferred。 |
...
- SSR/hydration 已覆盖 buffered async initial rendering 和顺序流式切片。`renderToStringAsync()`、
  `generateStaticSiteAsync()` 和 `hydrateAsync()` 已覆盖 async initial trees；`renderToStream()`
  以相同字节顺序顺序流式输出，样式在首次注册处内联、eager 启动、不处理消费者 backpressure，但乱序
  streaming SSR、
...
- 乱序 streaming SSR（顺序流式已通过 `renderToStream()` 实现）、显式 `{ recover: true }`
