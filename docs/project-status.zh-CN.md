...
npm `beta` 已发布版本是 `0.1.0-beta.9`（2026-09-08 发布）。它已经具备可运行的公共
...
- 包名：`@italone/solace`
- 仓库 package 版本：`0.1.0-beta.9`
- npm `latest` 已发布版本：`0.0.5`
- npm `beta` 已发布版本：`0.1.0-beta.9`
- npm dist-tags：`latest` 指向 `0.0.5`；`beta` 指向 `0.1.0-beta.9`
...
- 发布阶段：beta.9 于 2026-09-08 发布到 npm，增加了 SSR 挂起防护（三个 async SSR 入口的可选 `timeoutMs`，SSG 支持 site/route 级覆盖，`SolaceTimeoutError` 从根入口与 `./server` 导出，乱序流边界独立超时）与乱序边界加载后渲染失败修复；在此之上是 beta.8 的深度 `reactive()`（配 `shallowReactive()`）、DevTools event contract v1（router 事件、scheduler causes、trigger correlation）、hydration 属性不匹配加固（配 `textComparison`）、router-aware SSG、同步入口 router SSR、streaming backpressure、嵌套数组 children 展平与 keyed move-batch push 修复；初始运行时范围已完成，Router 的稳定切片已落地，但整体仍不是完整生产契约；SSR/hydration minimum loop，以及首个浏览器
...
| SSR/hydration   | buffered async、顺序与乱序流式加 router 组合 | `renderToStringAsync()` 会把完整 async initial tree 解析为 buffered HTML/styles；`hydrateAsync()` 会在触碰 server DOM 前完成准备，并支持显式 DOM mismatch recovery。现有同步 API 保留同步返回类型并拒绝未解析 async values。`renderToStream()` 已实现顺序流式（sequential）streaming SSR（beta）：字节顺序与 `renderToStringAsync().html` 一致，async component 解析前先刷新已完成前缀，样式在首次注册处内联发射；渲染 eager 启动并支持消费者 backpressure（流队列写满时生产挂起）。`renderToStream(source, { mode: "out-of-order" })` 增加乱序（out-of-order）streaming SSR（beta）：async 边界发射 `<!--so:b:N-->` fallback 标记，内联替换脚本按解析顺序在文档之后 flush，loader 失败时保留 fallback 且不拒绝流，客户端 hydration 前 DOM 已是最终状态。`createRouterServerContext()` 增加 request-scoped memory-router settlement、canonical snapshot serialization 和 verify-before-hydrate 组合。renderer-owned router SSR 已支持：`renderToStream()` 和 `renderToStringAsync()` 接受 `router` option，负责请求级 router settlement、注入其 `provides` 并附加内嵌 route snapshot script；`hydrateAsync(container, { router, routerIdentifyRecord })` 在水合前校验该 snapshot。同步入口 router SSR 也已支持：`renderToString()` 接受相同的 `router` option，经 `router.isReadySync()` settle（要求同步 guards；guard 返回 thenable 会抛出指向 async 入口的 `TypeError`），输出与 async 路径字节一致；`generateStaticSite()` 与 `generateStaticSiteAsync()` 均接受 route 级 `router` options（route 的 path 兼作 url）。同步 `hydrate()` 入口仍拒绝 `router` options。async SSR 入口另支持可选的 `timeoutMs` 挂起防护（SSG 支持 site 级配置并可用 route 级覆盖），超时会以 `SolaceTimeoutError`（根入口与 `@italone/solace/server` 均导出）拒绝；乱序流的 async 边界按各自超时处理并保留 fallback。 |
...
已经进入稳定切片，async renderer 入口、同步 `renderToString()` 和 SSG 的 route 级 `router` option 也已支持 renderer-owned router SSR，但 auth 和 permissions 仍是显式的 beta 范围边界；SSR/hydration 已覆盖 buffered async initial rendering，
...
- Router 仍处 beta，但稳定切片已经成形。当前 slice 已覆盖基础 SPA 工作流、`isReady()`、canonical snapshots 和 request-scoped server context；auth 和 permissions 仍未纳入，router options 与 route record fields 会被明确拒绝，避免把 route `meta` 误认为 enforcement；`renderToStream()`、`renderToStringAsync()` 和
  `hydrateAsync()` 已支持 renderer-owned router SSR，同步 `renderToString()`（经 `isReadySync()`）与两个 SSG 入口的 route 级 `router` option 也已支持，同步 `hydrate()` 的 router option 仍被推迟。
- SSR/hydration 已覆盖 buffered async initial rendering 和顺序/乱序流式切片。`renderToStringAsync()`、
  ...
  以相同字节顺序顺序流式输出，样式在首次注册处内联、eager 启动、支持消费者 backpressure；`renderToStream(source, { mode: "out-of-order" })` 以解析顺序的内联替换脚本流式输出 async 边界，配合 `defineAsyncComponent({ loader, fallback })` fallback 和不拒绝流的失败语义。Suspense/selective hydration 切片现已作为 beta 能力实现：内置 `Suspense` 组件在 CSR 与两种 streaming 模式下用一个 fallback 协调 async 子树（乱序模式下每棵 Suspense 子树一个 `so:b` 边界），`hydrateAsync(container, { selective: true })` 立即水合就绪部分、在 loader 解析后 patch 边界内容、落定后回放缓冲的交互，loader 失败时保留 fallback 且不 reject。renderer-owned router 切片现已作为 beta 能力实现：`renderToStream()` 和
  `renderToStringAsync()` 接受 `router` option（`{ url, routes, identifyRecord, configure? }`），
  构建请求级 memory router、等待 readiness、注入 server context `provides`，并把序列化 route snapshot
  以 `script#__solace-router-snapshot` 附加；`hydrateAsync(container, { router, routerIdentifyRecord })`
  等待 readiness、校验内嵌 snapshot 与客户端 route 一致、移除该 script 后才水合。router-aware selective
  hydration 会被拒绝。尽管如此，
  async suspension 后的 ambient instance
  APIs 和 initial hydration 后的 async update scheduling 仍保持
  deferred；router-aware SSG 已通过两个 SSG 入口的 route 级 `router` option 实现，同步入口 router
  SSR 也已通过 `renderToString()` 的 `router` option（`isReadySync()`，要求同步 guards）实现。production pipeline 行不再完全 deferred：三个 SSR renderer 都接受 `manifest` 加
...
4. 继续收敛 router beta API，但不急着扩 still-deferred 功能：auth 和 permissions 继续保持 deferred；renderer-owned `router` option（async 入口、经 `isReadySync()` 的同步 `renderToString()` 和 SSG route 级 options）以及显式 readiness、server context 与 snapshot verification workflow 继续保留。
