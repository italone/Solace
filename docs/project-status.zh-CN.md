...
npm `beta` 已发布版本是 `0.1.0-beta.8`（2026-09-08 发布）。它已经具备可运行的公共
...
- 包名：`@italone/solace`
- 仓库 package 版本：`0.1.0-beta.8`
- npm `latest` 已发布版本：`0.0.5`
- npm `beta` 已发布版本：`0.1.0-beta.8`
- npm dist-tags：`latest` 指向 `0.0.5`；`beta` 指向 `0.1.0-beta.8`
...
- 发布阶段：beta.8 于 2026-09-08 发布到 npm，在 beta.7 的根入口 `SolaceHydrationError` 导出之上，增加了深度 `reactive()`（配 `shallowReactive()`）、DevTools event contract v1（router 事件、scheduler causes、trigger correlation）、hydration 属性不匹配加固（配 `textComparison`）、router-aware SSG、同步入口 router SSR、streaming backpressure、嵌套数组 children 展平与 keyed move-batch push 修复；初始运行时范围已完成，Router 的稳定切片已落地，但整体仍不是完整生产契约；SSR/hydration minimum loop，以及首个浏览器
