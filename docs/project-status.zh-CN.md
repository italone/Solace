...
- 当前分支：`main`
- 本地分支状态：2026-07-28 执行 `git push origin main` 成功后，本地 `main` 与
  `origin/main` 已同步。后续发布或声明远端同步前，仍需重新运行 `git fetch origin main` 和
...
1. 发布工作前继续保持 `main` 与 `origin/main` 同步；最新 push 已成功，但发布前仍需重新运行
