...
| SFC compiler    | 稳定窄 SFC/Vite 契约 | `.solace` 解析、template codegen、runtime-helper style 注入、`@italone/solace/sfc`、`@italone/solace/vite`、Vite transform diagnostics、显式 `map: null` source-map policy、被拒绝的 plugin options、被拒绝的 `.solace?*` query transforms、被拒绝的 SFC block attributes 和被拒绝的自定义顶层 blocks 已文档化，并有 package-boundary tests 覆盖。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
SFC/Vite 已形成稳定窄公共契约，但仍然不是成熟 compiler 契约；Router 的 names、aliases、props 和 memory history 已经进入稳定切片，但 scroll behavior、auth、permissions 和 SSR/hydration integration 仍是显式的 beta 范围边界；SSR/hydration 目前仍只是 minimum loop，不是完整生产契约。
项目今天已经可用且文档齐全，但这些子系统仍然有意保持在冻结生产契约之前的范围内。

- 生态能力仍薄。没有一方 UI component library、稳定 plugin ecosystem、生产级 DevTools 发布形态或大型应用落地指南。
- SFC/Vite 是稳定窄契约，不是成熟 compiler 契约。当前公共契约只覆盖 `@italone/solace/vite`、`@italone/solace/sfc` 类型入口、文档化 block model、Vite transform diagnostics、显式 `map: null`、被拒绝的 block attributes、被拒绝的自定义顶层 blocks 和被拒绝的 `.solace?*` transforms；不能假设语法扩展或生成代码形状稳定。
...
- 超出当前稳定窄 SFC/Vite 契约的成熟 template/SFC compiler 契约。当前 `.solace` compiler 和 Vite plugin 已文档化为支持一个 `<template>`、可选一个 `<script>`、可选一个 `<style>`、拒绝 block attributes、拒绝自定义顶层 blocks、拒绝 `.solace?*` transforms、Vite transform diagnostics 和显式 `map: null` source-map policy；语法扩展和生成代码兼容性继续推迟。
...
2. 继续 harden 稳定窄 SFC/Vite contract，但不扩语法：公开面保持为 `@italone/solace/sfc`、`@italone/solace/vite`、Vite transform diagnostics、显式 `map: null`、当前文档化的 `.solace` block model，以及对 block attributes、自定义顶层 blocks、plugin options 和 `.solace?*` transforms 的显式拒绝。
3. 继续收敛 router beta API，但不急着扩 still-deferred 功能：scroll behavior、SSR/hydration 集成、auth 和 permissions 继续保持 deferred。
