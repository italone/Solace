...
- 当前分支：`main`
- 本地分支状态：当前按要求暂不 push，本地 `main` 相对 `origin/main` 为 `ahead 2`，这两个
  本地提交就是当前工作基线。`origin/main` 位于
  `20bbff9 docs: refresh local baseline status`；本地基线新增当前 router location boundary
  hardening 提交，以及当前 SFC/Vite query 和 router location shape boundary hardening 提交。后续发布、同步或声明远端状态前，
...
1. 在用户要求 push 前，继续明确当前本地 `ahead 2` 工作基线；不要在分支状态含糊时发布。
