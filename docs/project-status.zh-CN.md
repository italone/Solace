...
| SSR/hydration   | buffered async 加 router 组合           | `renderToStringAsync()` 会把完整 async initial tree 解析为 buffered HTML/styles；`hydrateAsync()` 会在触碰 server DOM 前完成准备，并支持显式 DOM mismatch recovery。现有同步 API 保留同步返回类型并拒绝未解析 async values。`createRouterServerContext()` 增加 request-scoped memory-router settlement、canonical snapshot serialization 和 verify-before-hydrate 组合。直接 `router` option 拒绝保持不变；streaming 和 renderer-owned router integration 仍 deferred。 |
