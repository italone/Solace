...
fallback, location-based browser/hash history listener deduplication, creation-time options/history adapter and route record/component validation, route redirect `"redirect-rejected"` errors for thrown and invalid redirect results, history-aware `RouterLink` href coverage, browser-owned `RouterLink` target/download handling, the lazy route `"lazy-load-failed"` regression contract, including active-route error
