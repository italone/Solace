...
line, while published npm `beta` is `0.1.0-beta.9` (published 2026-09-08).
...
- Repository package version: `0.1.0-beta.9`
- Published npm `latest`: `0.0.5`
- Published npm `beta`: `0.1.0-beta.9`
- npm dist-tags: `latest` points to `0.0.5`; `beta` points to `0.1.0-beta.9`
...
- Phase: beta.9 (published to npm 2026-09-08), which adds the SSR hang guard (opt-in `timeoutMs` on the three async SSR entries with site/route override on SSG, `SolaceTimeoutError` from root and `./server`, per-boundary out-of-order deadlines) and the out-of-order boundary post-load render-failure fix, on top of the beta.8 deep `reactive()` with `shallowReactive()`, the DevTools event contract v1 (router events, scheduler causes, trigger correlation), hydration attribute-mismatch hardening with `textComparison`, router-aware SSG, synchronous-entry router SSR, streaming backpressure, flattened nested array children, and the keyed move-batch push fix. The Router stable slice,
...
| SSR/hydration    | Buffered async, sequential plus out-of-order streaming, and router composition | `renderToStringAsync()` resolves complete async initial trees into buffered HTML/styles, while `hydrateAsync()` prepares before touching server DOM and supports explicit DOM mismatch recovery. Existing synchronous APIs retain synchronous return types and reject unresolved async values. `renderToStream()` implements sequential streaming SSR (beta): byte order matches `renderToStringAsync().html`, completed prefixes flush before async components resolve, and styles emit inline at first registration; it starts eagerly and applies consumer backpressure (production parks when the stream queue is full). `renderToStream(source, { mode: "out-of-order" })` adds out-of-order streaming SSR (beta): async boundaries emit `<!--so:b:N-->` fallback markers and inline replacement scripts flushed in resolution order after the document, loader failures keep the fallback without rejecting the stream, and the DOM is final before client hydration. `createRouterServerContext()` adds request-scoped memory-router settlement, canonical snapshot serialization, and verify-before-hydrate composition. Renderer-owned router SSR is now supported: `renderToStream()` and `renderToStringAsync()` accept a `router` option that settles a request-scoped router, injects its `provides`, and appends the embedded route snapshot script, and `hydrateAsync(container, { router, routerIdentifyRecord })` verifies that snapshot before hydrating. Synchronous-entry router SSR is also supported: `renderToString()` accepts the same `router` option settled via `router.isReadySync()` (synchronous guards required; thenable guard results throw a `TypeError` pointing at the async entries), with byte-parity with the async path, and both `generateStaticSite()` and `generateStaticSiteAsync()` accept route-level `router` options (the route path doubles as the url). The synchronous `hydrate()` entry still rejects `router` options. The async SSR entries additionally support an opt-in `timeoutMs` hang guard (site-level with route-level override on SSG) that rejects with the `SolaceTimeoutError` (root and `@italone/solace/server` exports); out-of-order stream boundaries time out individually and keep their fallbacks. |
...
supported on the async renderer entries, the synchronous `renderToString()` (via `isReadySync()`),
and route-level SSG `router` options, but auth, permissions, and
app-level SSG router integration remain explicit beta scope boundaries.
...
- Router remains beta, but the stable slice has taken shape. The current slice covers basic SPA
  ...
  route `meta` is not mistaken for enforcement. Renderer-owned router SSR is supported on
  `renderToStream()`, `renderToStringAsync()`, the synchronous `renderToString()`, and
  `hydrateAsync()`; route-level `router` options are supported on both SSG entries; the
  synchronous `hydrate()` entry keeps rejecting `router` options.
- SSR/hydration now covers buffered async initial rendering plus the sequential and out-of-order
  streaming slices.
  `renderToStringAsync()`,
  ...
  fallbacks and non-rejecting failure semantics. The renderer-owned router slice is now
  implemented as a beta feature: `renderToStream()` and `renderToStringAsync()` accept a `router`
  option (`{ url, routes, identifyRecord, configure? }`) that builds a request-scoped memory router,
  awaits readiness, injects the server context `provides`, and appends the serialized route
  snapshot as `script#__solace-router-snapshot`, while `hydrateAsync(container, { router,
routerIdentifyRecord })` awaits readiness, verifies the embedded snapshot against the client
  ...
  implemented on route-level `router` options for both SSG entries, and the synchronous
  `renderToString()` accepts the same `router` option backed by `router.isReadySync()`
  (synchronous guards required; thenable guard results throw a `TypeError` pointing at the async
  entries).
...
- A full first-party router contract. The current beta router covers static routes, dynamic params,
  ...
  context, and snapshot verification composition is supported, and the renderer entries now
  accept a renderer-owned `router` option (`renderToStream()`, `renderToStringAsync()`, and the
  synchronous `renderToString()` via `isReadySync()`), alongside route-level `router` options on
  both SSG entries.
  Direct renderer-owned SSR/hydration integration beyond that slice — the synchronous `hydrate()`
  entry — remains deferred. Auth and permissions remain
  deferred.
- Automatic hydration mismatch recovery beyond explicit `{ recover: true }`,
  renderer-owned router options for the synchronous `hydrate()` entry, ambient
...
4. **Continue narrowing the router beta API without adding still-deferred features**: keep
   auth and permissions deferred until separately
   designed; preserve the renderer-owned `router` option (async entries plus the synchronous
   `renderToString()` via `isReadySync()` and route-level SSG options) plus the explicit readiness,
   server-context, and snapshot-verification workflow.
