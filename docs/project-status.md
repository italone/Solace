...
fallback, creation-time route record/component validation, route redirect `"redirect-rejected"` errors for thrown and invalid redirect results, browser-owned `RouterLink` target/download handling, the lazy route `"lazy-load-failed"` regression contract, including active-route error
