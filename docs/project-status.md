...
- Router remains beta, but the stable slice has taken shape. The current slice covers basic SPA
  ...
  `renderToStream()`, `renderToStringAsync()`, and `hydrateAsync()`; SSG and synchronous-entry
  router options remain deferred.
...
- A full first-party router contract. The current beta router covers static routes, dynamic params,
  ...
  context, and snapshot verification composition is supported, and the async renderer entries now
  accept a renderer-owned `router` option.
  Direct renderer-owned SSR/hydration integration beyond that slice — router-aware SSG and the
  synchronous `renderToString()`/`hydrate()` entries — remains deferred. Auth and permissions remain
  deferred.
- Automatic hydration mismatch recovery beyond explicit `{ recover: true }`,
  renderer-owned router options for SSG and the synchronous SSR/hydration entries, ambient
...
4. **Continue narrowing the router beta API without adding still-deferred features**: keep
   router-aware SSG, synchronous-entry router options, auth, and permissions deferred until separately
   designed; preserve the renderer-owned `router` option plus the explicit readiness, server-context,
   and snapshot-verification workflow.
