...
| SFC compiler     | Stable narrow SFC/Vite contract | `.solace` parsing, template code generation, runtime-helper style injection, `@italone/solace/sfc`, `@italone/solace/vite`, Vite transform diagnostics, explicit `map: null` source-map policy, rejected plugin options, rejected `.solace?*` query transforms, rejected SFC block attributes, and rejected custom top-level blocks are documented and covered by package-boundary tests.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
SFC/Vite now has a stable narrow public contract, but it remains deliberately short of a mature
compiler contract. Router names, aliases, props, and memory history are now in the stable slice, but
scroll behavior, auth, permissions, and SSR/hydration integration are still explicit beta scope
boundaries. SSR/hydration remains a minimum loop, not a full production contract. The project is
usable and documented today, but these subsystems are still intentionally scoped short of a frozen
production contract.
...
- SFC/Vite is a stable narrow contract, not a mature compiler contract. The current public contract
  covers only `@italone/solace/vite`, the `@italone/solace/sfc` type entry, the documented block
  model, Vite transform diagnostics, explicit `map: null`, rejected block attributes, rejected
  custom top-level blocks, and rejected `.solace?*` transforms; syntax expansion and generated code
  shape should not be treated as stable.
...
- A mature template/SFC compiler contract beyond the current stable narrow SFC/Vite contract. The
  current `.solace` compiler and Vite plugin are documented for one `<template>`, optional one
  `<script>`, optional one `<style>`, rejected block attributes, rejected custom top-level blocks,
  rejected `.solace?*` transforms, Vite transform diagnostics, and explicit `map: null` source-map
  policy; syntax expansion and generated-code compatibility remain deferred.
...
2. **Harden the stable narrow SFC/Vite contract without syntax expansion**: keep the public surface
   limited to `@italone/solace/sfc`, `@italone/solace/vite`, Vite transform diagnostics, explicit
   `map: null`, the documented `.solace` block model, and explicit rejection of block attributes,
   custom top-level blocks, plugin options, and `.solace?*` transforms.
