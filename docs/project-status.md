...
| SSR/hydration    | Buffered async plus router composition | `renderToStringAsync()` resolves complete async initial trees into buffered HTML/styles, while `hydrateAsync()` prepares before touching server DOM and supports explicit DOM mismatch recovery. Existing synchronous APIs retain synchronous return types and reject unresolved async values. `createRouterServerContext()` adds request-scoped memory-router settlement, canonical snapshot serialization, and verify-before-hydrate composition. Direct `router` option rejections remain unchanged; streaming and renderer-owned router integration remain deferred. |
