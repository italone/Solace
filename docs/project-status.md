...
direction. Router names, aliases, props, and memory history are now in the stable slice, but scroll
behavior, auth, permissions, and SSR/hydration integration are still explicit beta scope boundaries.
...
4. **Continue narrowing the router beta API without adding still-deferred features**: keep scroll
   behavior, SSR/hydration integration, auth, and permissions deferred until separately designed.
5. **Keep public API gates mandatory**: `pnpm release:readiness`, `pnpm package:smoke`, and `pnpm test:e2e` must run for public API changes.
