...
| Router           | Beta next slice stabilized     | Matcher, history adapters, query helpers, nested route chains, redirects, global and route-level guards, initial history navigation pipeline, stale async navigation result protection, rejected-guard history recovery, invalid history location recovery, invalid initial history fallback, `lazyRoute()` components, surfaced `lazy-load-failed` errors, `RouterLink`, nested `RouterView`, root exports, deferred API boundaries, package export coverage, packed-consumer smoke, and expanded `router-basic` e2e coverage exist.                    |
...
fallback, and the lazy route `"lazy-load-failed"` regression contract. It did not rerun coverage,
package smoke outside `pnpm quality`, benchmarks, browser e2e, DevTools extension e2e, or the full
`release:check`. Run the commands again before any future completion, merge, or release claim.
