...
- [x] **Step 1: 创建 `src/renderer/unmount.ts`**
...
- [x] **Step 2-4: 同 Task 1**(diff.ts 改导入 → `pnpm typecheck && pnpm vitest run tests/unit` 全绿)

- [x] **Step 5: Commit** `git commit -m "refactor: extract unmount module from diff"`
...
- [x] **Step 1: 创建 `src/renderer/children.ts`**
...
- [x] **Step 4: Commit** `git commit -m "refactor: extract children diff module"`
...
- [x] **Step 1: 移出 `emitComponentDevtoolsEvent`、`emitRendererElementDevtoolsEvent`(diff.ts:869-897)到 `devtools-events.ts`**
...
- [x] **Step 4: Commit** `git commit -m "refactor: extract renderer devtools events"`
...
- [x] **Step 1: 把 config.mjs 的导出常量原样移到 runner.mjs 顶部并 `export`**(供测试导入),删除 runner 中的 `import ... from "./X-config.mjs"`
- [x] **Step 2: 更新对应测试**:`tests/unit/scripts/X.test.ts` 中 `from "../../scripts/X-config.mjs"` 改为 `from "../../scripts/X-runner.mjs"`,断言不动
- [x] **Step 3: 删除 config 文件对**
- [x] **Step 4: 验证该组**

Run: `pnpm vitest run tests/unit/scripts/X.test.ts`
Expected: PASS

- [x] **Step 5: 每组合并成一个 commit** `git commit -m "refactor: inline <name> config into runner script"`

全部完成后:

- [x] **Step 6: 全量验证** `pnpm typecheck && pnpm lint && pnpm test`(注意 tests/unit/scripts 有 15 个测试文件,8 个需改导入路径)
- [x] **Step 7: 抽查一个门禁真实可用** `pnpm release:contract:check`
...
- [x] **Step 1: 在 docs/project-status.md 边界小节开头加一段定位说明(中英文档同步):**
...
- [x] **Step 2: 确认 docs/roadmap.md 已有对应条目,若无则补一条「Streaming SSR / Suspense / selective hydration — revisit after 1.0, requires design doc」**
- [x] **Step 3: 验证文档一致性测试** `pnpm vitest run tests/unit/docs`
      Expected: PASS(docs 内容有测试锁定,若断言失败按测试期望修正表述)

- [x] **Step 4: Commit** `git commit -m "docs: frame feature boundary as deliberate scope"`
...
- [x] **Step 2: 移出 git 跟踪但保留本地文件**

```bash
git rm -r --cached solace-project-log solace-project-plan
```

- [x] **Step 3: 删除可再生产物目录(coverage/test-results/dist 均可由命令重新生成)**
...
- [x] **Step 4: 验证** `git status` 只显示 .gitignore 与两个目录的删除;`pnpm vitest run tests/unit/ci-workflow.test.ts` PASS
- [x] **Step 5: Commit** `git commit -m "chore: stop tracking ai workflow logs and clean artifacts"`

## Task 8: 终验

- [x] **Step 1:** `pnpm quality`(format:check + contract + build + 双 typecheck + lint + test + test:package)
- [x] **Step 2:** `wc -l src/renderer/diff.ts` 确认 <350;`ls scripts | wc -l` 确认 ≤24
- [x] **Step 3:** `git log --oneline` 确认各任务独立提交
- [x] **Step 4:** 推送前与用户确认,再 `git push origin main`(SSH 已配好)
